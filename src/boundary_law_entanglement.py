...
# Set U to a value near the 2D phase transition
U = 16.666667
    
# Choose range of betas
beta_list = [2,4,6,8,10,12,16,20,24] # For L=256
...
for lA_sector_wanted in range(1,l_max+1):
    S2_plot = []
    S2_err_plot = []
    ...
    with open("../ProcessedData/"+str(D)+"D_%d_%d_%d_%.6f_%.6f_betas_%d_S2.dat"%(L,N,int(lA_sector_wanted),U,t,bin_size),"wb") as processed_data:
        np.savetxt(processed_data,np.c_[beta_list,S2_plot,S2_err_plot],delimiter=" ",fmt="%.16f",header="BH Parameters: L=%d,N=%d,D=%d,l=%d,U=%.6f,t=%.6f,bin_size=%d \n beta            <S2>               StdErr."%(L,N,D,int(lA_sector_wanted),U,t,bin_size))
